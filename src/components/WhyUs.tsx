...
export default function WhyUs() {
  ...
  return (
    <section id="about" className="relative bg-panel py-24">
        ...
        <div className="grid items-center gap-12 lg:grid-cols-2">
          <Reveal>
            <div>
              ...
              <h2 className="mt-3 text-3xl font-black text-ink sm:text-5xl">{t('about.title')}</h2>
              <p className="mt-5 leading-relaxed text-ink-mid">{t('about.p1')}</p>
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                {points.slice(0, 4).map((point) => (
                  <div key={point.key} className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="font-bold text-ink">{t(`about.${point.key}.title`)}</div>
                      <div className="mt-1 text-sm text-ink-soft">{t(`about.${point.key}.desc`)}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {points.slice(4).map((point, index) => (
              <Reveal key={point.key} delay={index * 0.08}>
                <div className="h-full rounded-2xl border border-line/8 bg-surface/70 p-6 transition hover:border-amber-brand/40">
                  <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-amber-brand text-navy-950">
                    <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                      {icons[point.key]}
                    </svg>
                  </div>
                  <div className="mt-4 font-bold text-ink">{t(`about.${point.key}.title`)}</div>
                  <div className="mt-2 text-sm leading-relaxed text-ink-soft">{t(`about.${point.key}.desc`)}</div>
                </div>
              </Reveal>
            ))}
            <Reveal delay={0.2} className="sm:col-span-2">
              <div className="rounded-2xl border border-amber-brand/30 bg-gradient-to-r from-amber-brand/15 to-transparent p-6">
                <div className="font-bold text-ink">{t('about.promise.title')}</div>
                <div className="mt-2 text-sm leading-relaxed text-ink-mid">{t('about.promise.desc')}</div>
              </div>
            </Reveal>
          </div>
        </div>
